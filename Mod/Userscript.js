...
(function() {
    ...
    const configuracoesHTML = `<!DOCTYPE html>
    <html lang="pt-BR">
    <head>
        <meta charset="UTF-8">
        ...
        <style>
            body{background-color:#222;margin:0;padding:0}.settings-panel{position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background-color:#222;border:1px solid #444;border-radius:8px;padding:20px;box-shadow:0 0 30px rgba(0,0,0,0.8);width:90%;max-width:800px;max-height:90vh;overflow:auto;color:#eee;z-index:10000}.settings-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:20px;padding-bottom:10px;border-bottom:1px solid #444}.settings-title{margin:0;color:#FFA500;font-size:1.3em}.close-button{background:none;border:none;color:#ccc;font-size:1.5em;cursor:pointer;transition:color 0.2s}.close-button:hover{color:#FFA500}.settings-section{margin-bottom:25px;padding-bottom:15px;border-bottom:1px solid #444}.section-title{color:#FFA500;font-size:1.1em;margin-bottom:15px}.settings-row{display:flex;flex-wrap:wrap;gap:15px;margin-bottom:15px;align-items:center}.settings-label{width:150px;color:#ccc}.settings-input{flex:1;min-width:200px;padding:8px 12px;background-color:#333;color:#fff;border:1px solid #444;border-radius:6px}.settings-button-group{display:flex;gap:10px;margin-top:10px}.primary-button{padding:10px 16px;background-color:#444;color:#fff;border:1px solid #555;border-radius:6px;cursor:pointer;font-size:14px;transition:all 0.2s ease}.primary-button:hover{background-color:#555;border-color:#FFA500}.secondary-button{padding:10px 16px;background:linear-gradient(135deg,#444,#555);color:#fff;border:none;border-radius:6px;cursor:pointer;transition:all 0.3s ease}.secondary-button:hover{transform:translateY(-2px);box-shadow:0 6px 15px rgba(0,0,0,0.4)}.preview-container{margin-top:15px;padding:15px;background-color:#333;border-radius:6px;border:1px solid #444}.preview-title{margin-top:0;margin-bottom:10px;font-size:0.9em;color:#ccc}.preview-image{max-width:100%;max-height:200px;display:block;margin:0 auto;border-radius:4px}.color-picker-container{display:flex;align-items:center;gap:10px}.color-preview{width:30px;height:30px;border-radius:50%;border:2px solid #555;cursor:pointer}.theme-actions{display:flex;gap:10px}.theme-sample{width:100%;height:40px;border-radius:6px;margin-top:10px;display:flex;align-items:center;justify-content:center;font-weight:bold;border:1px solid #444}.switch{position:relative;display:inline-block;width:60px;height:34px}.switch input{opacity:0;width:0;height:0}.slider{position:absolute;cursor:pointer;top:0;left:0;right:0;bottom:0;background-color:#ccc;transition:.4s;border-radius:34px}.slider:before{position:absolute;content:"";height:26px;width:26px;left:4px;bottom:4px;background-color:white;transition:.4s;border-radius:50%}input:checked+.slider{background-color:#FFA500}input:checked+.slider:before{transform:translateX(26px)}.info-text{font-size:0.85em;color:#aaa;margin-top:5px;line-height:1.4}.warning-text{font-size:0.85em;color:#FFA500;margin-top:5px;line-height:1.4}.random-color-btn,.remove-btn{padding:8px 12px;background-color:#333;color:#fff;border:1px solid #555;border-radius:6px;cursor:pointer;font-size:14px;transition:all 0.2s ease}.random-color-btn:hover,.remove-btn:hover{background-color:#444;border-color:#FFA500}.error-text{font-size:0.85em;color:#ff6b6b;margin-top:5px;line-height:1.4;display:none}@media (max-width:768px){.settings-row{flex-direction:column;align-items:flex-start}.settings-label{width:100%}.settings-input{width:100%}}
        </style>
    </head>
    <body>
        ...
        <script>
            let siteBgDataUrl='';let chatBgDataUrl='';let themeColor='#FFA500';let adblockerEnabled=false;let divulgarEnabled=false;let allSeasonsEnabled=false;
            function loadSettings(){const savedSettings=localStorage.getItem('firedeluxe_configuracoes');if(savedSettings){const settings=JSON.parse(savedSettings);if(settings.siteBgImage){siteBgDataUrl=settings.siteBgImage;document.getElementById('siteBgPreview').src=siteBgDataUrl;document.getElementById('siteBgPreviewContainer').style.display='block'}if(settings.chatBgImage){chatBgDataUrl=settings.chatBgImage;document.getElementById('chatBgPreview').src=chatBgDataUrl;document.getElementById('chatBgPreviewContainer').style.display='block'}if(settings.themeColor){themeColor=settings.themeColor;document.getElementById('themeColor').value=themeColor;updateThemeSample(themeColor)}if(settings.adblocker){adblockerEnabled=settings.adblocker==='on';document.getElementById('adblockerToggle').checked=adblockerEnabled}if(settings.divulgar){divulgarEnabled=settings.divulgar==='on';document.getElementById('divulgarToggle').checked=divulgarEnabled}if(settings.allSeasons){allSeasonsEnabled=settings.allSeasons==='on';document.getElementById('allSeasonsToggle').checked=allSeasonsEnabled}if(settings.email){document.getElementById('automationEmail').value=settings.email}if(settings.senha){document.getElementById('automationPassword').value=settings.senha}}}
            function updateThemeSample(color){const sample=document.getElementById('themeSample');sample.style.backgroundColor=hexToRgba(color,0.2);sample.style.color=color;sample.style.borderColor=color}
            ...
            function handleImageUpload(inputId,previewId,containerId,errorId){const file=document.getElementById(inputId).files[0];const errorElement=document.getElementById(errorId);errorElement.style.display='none';if(file){if(file.size>5*1024*1024){errorElement.style.display='block';document.getElementById(inputId).value='';return}const reader=new FileReader();reader.onload=function(event){if(inputId==='siteBgImage'){siteBgDataUrl=event.target.result}else if(inputId==='chatBgImage'){chatBgDataUrl=event.target.result}document.getElementById(previewId).src=event.target.result;document.getElementById(containerId).style.display='block'};reader.onerror=function(){errorElement.style.display='block'};reader.readAsDataURL(file)}}
            function removeImage(inputId,previewId,containerId,errorId){document.getElementById(inputId).value='';document.getElementById(previewId).src='';document.getElementById(containerId).style.display='none';document.getElementById(errorId).style.display='none';if(inputId==='siteBgImage'){siteBgDataUrl=''}else if(inputId==='chatBgImage'){chatBgDataUrl=''}}
            document.getElementById('randomColorBtn').addEventListener('click',function(){const randomColor=getRandomColor();themeColor=randomColor;document.getElementById('themeColor').value=randomColor;updateThemeSample(randomColor)});
            document.getElementById('themeColor').addEventListener('input',function(e){themeColor=e.target.value;updateThemeSample(themeColor)});
            document.getElementById('removeThemeColor').addEventListener('click',function(){themeColor='#FFA500';document.getElementById('themeColor').value=themeColor;updateThemeSample(themeColor)});
            document.getElementById('siteBgImage').addEventListener('change',function(){handleImageUpload('siteBgImage','siteBgPreview','siteBgPreviewContainer','siteBgError')});
            document.getElementById('removeSiteBg').addEventListener('click',function(){removeImage('siteBgImage','siteBgPreview','siteBgPreviewContainer','siteBgError')});
            document.getElementById('chatBgImage').addEventListener('change',function(){handleImageUpload('chatBgImage','chatBgPreview','chatBgPreviewContainer','chatBgError')});
            document.getElementById('removeChatBg').addEventListener('click',function(){removeImage('chatBgImage','chatBgPreview','chatBgPreviewContainer','chatBgError')});
            document.getElementById('saveSettings').addEventListener('click',function(){const settings={themeColor:themeColor,siteBgImage:siteBgDataUrl,chatBgImage:chatBgDataUrl,adblocker:document.getElementById('adblockerToggle').checked?'on':'off',divulgar:document.getElementById('divulgarToggle').checked?'on':'off',allSeasons:document.getElementById('allSeasonsToggle').checked?'on':'off',email:document.getElementById('automationEmail').value,senha:document.getElementById('automationPassword').value};localStorage.setItem('firedeluxe_configuracoes',JSON.stringify(settings));window.close()});
            document.getElementById('resetSettings').addEventListener('click',function(){if(confirm('Tem certeza que deseja redefinir todas as configurações para os valores padrão?')){localStorage.removeItem('firedeluxe_configuracoes');themeColor='#FFA500';document.getElementById('themeColor').value=themeColor;updateThemeSample(themeColor);document.getElementById('siteBgImage').value='';document.getElementById('siteBgPreview').src='';document.getElementById('siteBgPreviewContainer').style.display='none';siteBgDataUrl='';document.getElementById('siteBgError').style.display='none';document.getElementById('chatBgImage').value='';document.getElementById('chatBgPreview').src='';document.getElementById('chatBgPreviewContainer').style.display='none';chatBgDataUrl='';document.getElementById('chatBgError').style.display='none';document.getElementById('adblockerToggle').checked=false;document.getElementById('divulgarToggle').checked=false;document.getElementById('allSeasonsToggle').checked=false;document.getElementById('automationEmail').value='';document.getElementById('automationPassword').value='';window.close()}});
            document.getElementById('closeSettings').addEventListener('click',function(){window.close()});
            loadSettings();
        </script>
    </body>
    </html>`;
